...
sequenceDiagram
    ...
    V->>V: policy.itrustee trusted_uuids + trusted_ta_img_hex comparison
    V-->>RP: EAR JWT (ES256)
...
verifier-side `[policy.itrustee]`:

| key | description |
|---|---|
| `trusted_uuids` | Trusted TA UUID whitelist (empty = skip check) |
| `trusted_ta_img_hex` | Trusted TA image measurement whitelist (hex, empty = skip check) |

When `trusted_uuids` is non-empty, the `uuid` in claims must match one of the entries.
When `trusted_ta_img_hex` is non-empty, the `ta_img` in claims must match one of the entries.
Both checks are independent — skipping one does not skip the other.
...
- No itrustee-hydra stacking path
