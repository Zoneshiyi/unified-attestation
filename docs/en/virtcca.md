...
sequenceDiagram
    ...
    V->>V: policy.virtcca trusted_rim_hex comparison
    V-->>RP: EAR JWT (ES256)
...
verifier-side `[policy.virtcca]`:

| key | description |
|---|---|
| `trusted_rim_hex` | Trusted RIM whitelist (hex, empty = skip check) |

When `trusted_rim_hex` is non-empty, the `rim` field in claims must match one of the entries.
...
- No virtcca-hydra stacking path.
