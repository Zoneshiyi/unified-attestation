...
sequenceDiagram
    ...
    V->>V: policy.itrustee trusted_uuids + trusted_ta_img_hex 比对
    V-->>RP: EAR JWT (ES256)
...
verifier 侧 `[policy.itrustee]`：

| 键 | 说明 |
|---|---|
| `trusted_uuids` | 可信 TA UUID 白名单（空 = 跳过检查） |
| `trusted_ta_img_hex` | 可信 TA image 度量值白名单（hex，空 = 跳过检查） |

`trusted_uuids` 非空时，claims 中的 `uuid` 必须在白名单内。`trusted_ta_img_hex` 非空时，claims 中的 `ta_img` 必须在白名单内。两项检查独立——跳过一项不影响另一项。
