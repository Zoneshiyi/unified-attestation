...
证据采集：attester 通过 AA REST 获取 `{report, ima_log}` 并附加 nonce。
host 端验证：解析 report JSON 提取 TA 度量值注入 evidence，完整验签需 libteeverifier.so（部署时按需接入）。
...
证据采集：attester 通过 AA REST 获取 `{evidence, dev_cert, event_log}` 并附加 nonce。
host 端验证：完整验签需 OpenSSL + cose + ciborium（CBOR/COSE 解码 + HW 证书链验证），部署时按需接入。
