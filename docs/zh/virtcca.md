...
sequenceDiagram
    ...
    V->>V: policy.virtcca trusted_rim_hex 比对
    V-->>RP: EAR JWT (ES256)
...
verifier 侧 `[policy.virtcca]`：

| 键 | 说明 |
|---|---|
| `trusted_rim_hex` | 可信 RIM 白名单（hex，空 = 跳过检查） |

`trusted_rim_hex` 非空时，claims 中的 `rim` 字段必须在白名单内。
