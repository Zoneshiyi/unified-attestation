...
`tee_type = tdx-hydra` 时，attester 同时携带 TDX quote / collateral 与 Groth16
证明，共用同一 nonce。校验顺序（全部在 wasm 内）：

1. dcap-qvl 完整链验签（同 TDX-only）
2. quote.report_data[..32] == expected_report_data
3. quote.mr_config_id == expected_init_data_hash（host 透传）
...
模板：`config/verifier-tdx-hydra.toml` + `config/attester-tdx-hydra.toml`。
...
sequenceDiagram
    ...
    participant TEE as Intel TDX 硬件
...
在 TDX-only 步骤基础上多一步 hydra trusted setup，启动配置改为 `*-tdx-hydra.toml`：
