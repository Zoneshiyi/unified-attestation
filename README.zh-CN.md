...
支持 mock / CCA / CSV / TDX / iTrustee / VirtCCA 八条平台路径，每条路径均可叠加 hydra Groth16 证明，
在白名单中证明设备身份且不暴露具体索引。
...
四条核心机制（详细论证见 [docs/zh/design.md](docs/zh/design.md)）：
...
- CCA / CSV / TDX 证据采集依赖 [guest-components](https://github.com/SmartTree-zq1997/guest-components)（`attestation-agent` + `api-server-rest`）
...
[docs/zh/protocol.md](docs/zh/protocol.md)。

### 配置项

verifier / attester 全部 key 与默认值：[docs/zh/config.md](docs/zh/config.md)。

### hydra 子模块

Groth16 + shrubs whitelist 累积器的电路定义、public input 顺序、trusted setup 用法：
[docs/zh/hydra.md](docs/zh/hydra.md)。
...
│   ├── itrustee/                iTrustee（wasm 内 nonce 绑定 + TA 度量值提取）
│   └── virtcca/                 VirtCCA（wasm 内 nonce 绑定 + token 元数据透传）
├── config/                      配置模板
├── scripts/                     构建与一键脚本
└── docs/
    ├── en/                      英文文档
...
| 主题                                | 文档                                       |
| --------------------------------- | ---------------------------------------- |
| 设计要点（四条核心机制）                      | [docs/zh/design.md](docs/zh/design.md)         |
| 协议层（gRPC 服务、消息、EAR 格式）            | [docs/zh/protocol.md](docs/zh/protocol.md)     |
| 配置项参考（verifier / attester 全部 key） | [docs/zh/config.md](docs/zh/config.md)         |
| 操作手册（构建、脚本、工具命令）                  | [docs/zh/operations.md](docs/zh/operations.md) |
